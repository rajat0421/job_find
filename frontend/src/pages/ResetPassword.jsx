        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-7 shadow-sm">
          {success ? (
            <div className="text-center py-4">
              <p className="text-sm text-emerald-600 font-medium">Password reset! Redirecting to login...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-3">
                  OTP from your email {prefillEmail && <span className="text-slate-400 font-normal">({prefillEmail})</span>}
                </label>
                <div className="flex gap-2 justify-between" onPaste={handlePaste}>
                  {digits.map((d, i) => (
                    <input
                      ...
                      className="w-11 h-12 text-center text-lg font-semibold border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-slate-900"
                    />
                  ))}
                </div>
              </div>
              ...
              <p className="text-center text-xs text-slate-400">
                Didn't get the OTP?{' '}
                <Link to="/forgot-password" className="text-indigo-600 hover:text-indigo-700">Resend</Link>
              </p>
            </form>
          )}
        </div>
